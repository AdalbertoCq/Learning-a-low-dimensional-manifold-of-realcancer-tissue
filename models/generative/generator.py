...
def generator_resnet_style_150(w_input, image_channels, layers, spectral, activation, reuse, is_train, normalization, init='xavier', noise_input_f=False, regularizer=None, cond_label=None, attention=None, up='upscale'):
	# channels = [32, 64, 128, 256, 512, 1024, 2048]
	channels = [32, 64, 128, 256, 512, 1024]
	...
	with tf.variable_scope('generator', reuse=reuse):
		...
		# 150 model changes.
		net = convolutional(inputs=net, output_channels=reversed_channel[layer], filter_size=4, stride=1, padding='VALID', conv_type='convolutional', spectral=spectral, init=init, regularizer=regularizer, scope=layer+1)
		if noise_input_f: net = noise_input(inputs=net, scope=layer+1)
		net = normalization(inputs=net, training=is_train, c=label, spectral=spectral, scope=layer+1)
		net = activation(net)

		net = convolutional(inputs=net, output_channels=reversed_channel[layer], filter_size=4, stride=1, padding='VALID', conv_type='convolutional', spectral=spectral, init=init, regularizer=regularizer, scope=layer+2)
		if noise_input_f: net = noise_input(inputs=net, scope=layer+2)
		net = normalization(inputs=net, training=is_train, c=label, spectral=spectral, scope=layer+2)
		net = activation(net)

		net = convolutional(inputs=net, output_channels=reversed_channel[layer], filter_size=5, stride=1, padding='VALID', conv_type='convolutional', spectral=spectral, init=init, regularizer=regularizer, scope=layer+3)
		if noise_input_f: net = noise_input(inputs=net, scope=layer+3)
		net = normalization(inputs=net, training=is_train, c=label, spectral=spectral, scope=layer+3)
		net = activation(net)
...
def generator_resnet_style2(z_input, image_channels, layers, spectral, activation, reuse, is_train, normalization, init='xavier', noise_input_f=False, regularizer=None, cond_label=None, attention=None, up='upscale'):
	channels = [32, 64, 128, 256, 512, 1024]
	reversed_channel = list(reversed(channels[:layers]))
	...
	with tf.variable_scope('generator', reuse=reuse):
		...
		for layer in range(layers):
			...
			net = conv_mod(inputs=net, label=label, output_channels=reversed_channel[layer], filter_size=2, stride=2, padding='SAME', conv_type=up, spectral=spectral, init=init, regularizer=regularizer, scope=layer)
			if noise_input_f:
