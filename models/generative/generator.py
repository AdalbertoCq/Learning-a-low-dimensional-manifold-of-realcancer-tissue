...
def generator_resnet_style(z_input, image_channels, layers, spectral, activation, reuse, is_train, normalization, init='xavier', noise_input_f=False, regularizer=None, cond_label=None, attention=None, up='upscale', constant=False):
	channels = [32, 64, 128, 256, 512, 1024]
	# For resolution 448
	...
	with tf.variable_scope('generator', reuse=reuse):
		...
		if not constant:
			...
			constant = tf.constant(np.random.rand(1, 7, 7, 256), shape=(1, 7, 7, 256), dtype=tf.float32)
			dim = tf.shape(constant)[0]
			net = tf.tile(constant, [dim, 1, 1, 1])
			print('Constant Input: Shape', net.get_shape())
