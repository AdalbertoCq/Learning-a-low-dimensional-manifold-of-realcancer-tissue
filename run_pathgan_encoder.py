...
with tf.Graph().as_default():
    pathgan_encoder = PathologyGAN_Encoder(data=data, z_dim=z_dim, layers=layers, use_bn=use_bn, alpha=alpha, beta_1=beta_1, init=init, regularizer_scale=regularizer_scale, attention=attention,
    									   spectral=spectral, noise_input_f=noise_input_f, learning_rate_g=learning_rate_g, learning_rate_d=learning_rate_d, learning_rate_e=learning_rate_e, 
    									   beta_2=beta_2, n_critic=n_critic, gp_coeff=gp_coeff, loss_type=loss_type, model_name=model)
    losses = pathgan_encoder.train(epochs, data_out_path, data, restore, print_epochs=10, n_images=10, show_epochs=None)
