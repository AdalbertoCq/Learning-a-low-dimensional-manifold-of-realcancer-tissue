...
if 'vgh' in dataset or 'nki' in dataset:
	# Generate CRImage features from fake images. 
	crimage_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'CRImage')
	main_evaluation_path = gen_hdf5_path.split('hdf5_')[0]

	# Generated images CRImage.
	cri_name = 'crimage_train.txt'
	crimage_file = os.path.join(main_evaluation_path, cri_name)
	if not os.path.isfile(crimage_file):
	    crimage_command = 'cd %s; Rscript --vanilla crimage_style.r %s %s %s %s %s' % (crimage_path, main_evaluation_path, num_samples, 'generated_images', 'gen_', cri_name)
