...
class Preprocessor:
    def __init__(self,  patch_h, patch_w, n_channels, dataset, marker, labels=None, overlap=False, save_img=False, threshold=215, do_augment=True, project_path=os.getcwd()):
        ...
        # Flag for doing data augmenation.
    ...
    # Per filename 
    def sample_patches(self, filename, do_augment=True):
        patches = set()
        display = False
        ...
        num_y = int(height//(self.patch_h))
        num_x = int(width//(self.patch_w))
    ...
    def get_sets(self):
        try:
            ...
            sets = self.split_data_into_sets([1., 0.])
